                ...
                <div class="">
                    ...
                    <div class="card-datatable text-nowrap ">
                        <table class="dt-complex-header table table-bordered" style="border-color: #d4d8dd">
                            ...
                            $bookingLists = App\Models\Booking::orderBy('id','desc')->get();
                            @endphp
                            ...
                            <tbody>
                                ...
                                @php 
                                $userData = App\Models\User::where('id',$bookingList->id)->first();
                                @endphp
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>{{$userData->name}}</td>
                                    <td>{{$userData->email}}</td>
                                    <td>{{$bookingList->booking_date}} & {{$bookingList->booking_time}}</td>
                                    @php 
                                        $vendor =  App\Models\Gallery::whereIn('gallery.id',json_decode($bookingList->vendor))->join('category','category.id','=','gallery.category_id')->select('gallery.*','category.title as cname')->get();
                                    @endphp
                                    <td>
                                        @foreach($vendor as $ven)
                                        NAME     : {{$ven->title}}<br />
                                        AMOUNT   : Rs.  {{$ven->amount}}<br />
                                        CATEGORY : {{$ven->cname}}<br />
                                        CONTACT  : {{$ven->number}}
                                        <hr>
                                        @endforeach
                                    </td>
...
<script src="{{URL::asset('admin/back_js/booking.js')}}"></script>
